...
public class SimRobot {
    ...
    static UpdateMessage generateUpdate(int xPos, int yPos, int robotHeading, int towerHeading, int s1, int s2, int s3, int s4) {
        ...
        try {
            ds.writeShort(xPos);
            ds.writeShort(yPos);
            ds.writeShort(robotHeading);
            ds.writeShort(towerHeading);
            ds.writeByte(s1);
            ds.writeByte(s2);
            ds.writeByte(s3);
            ds.writeByte(s4);
            um = new UpdateMessage(os.toByteArray());
        } catch(Exception e) {
    ...
    HandshakeMessage generateHandshake() {
        ...
        try {
            ...
            ds.writeShort(robotWidth);
            ds.writeShort(robotLength);
            int i;
            for(i=0;i<2;i++) ds.write( (byte) toweroffset[i] );
            ds.write(axleoffset);
            for(i=0;i<4;i++) ds.write(sensoroffset[i]);
            for(i=0;i<4;i++) ds.writeShort(irheading[i]);
            ds.writeShort(messageDeadline);
            hm = new HandshakeMessage(os.toByteArray());
        } catch(Exception e) {
